...
def _show_readonly(case):
    ...
    with col2:
        ...
        st.write(f"対応期限：{case['due_date'] or '（未設定）'}")
        st.write(f"完了日：{case['completed_date'] or '（未設定）'}")
        st.write(f"期限超過：{overdue_label(case['overdue']) or '（なし）'}")
...
def render_case_detail(case_id):
    render_page_header(
        "案件詳細",
        "案件の確認・編集ができます。ステータスを完了にすると進捗率は100%、完了日は自動設定されます。",
    )
    ...
    if success_id:
        show_success_with_id("案件を更新しました。", "案件ID", success_id)
    ...
    st.caption("ステータスを完了にすると進捗率は100%になり、完了日が未入力なら当日が設定されます。")
    ...
    with st.form(f"case_edit_form_{case['case_id']}"):
        ...
        with col2:
            priority = st.selectbox(
                "優先度（必須）",
                PRIORITIES,
                index=choice_index(PRIORITIES, case["priority"]),
            )
        ...
        with col2:
            sub_assignee = st.selectbox(
                "副担当者",
                ASSIGNEES,
                index=choice_index(ASSIGNEES, case["sub_assignee"] or UNASSIGNED_ASSIGNEE),
            )
        ...
        with col1:
            status = st.selectbox(
                "ステータス（必須）",
                CASE_STATUSES,
                index=choice_index(CASE_STATUSES, case["status"]),
            )
            progress = st.slider(
                "進捗率",
                min_value=0,
                max_value=100,
                value=int(case["progress"] or 0),
            )
        with col2:
            start_date = st.date_input("開始日（必須）", value=parse_date(case["start_date"]))
            due_date = st.date_input("対応期限", value=parse_date(case["due_date"]))
        ...
        submitted = st.form_submit_button("案件を更新")
